...
constructor(string memory _tld) ERC721("BLOCC Name Service", "BLOCC") payable {
  ...
  function register(string calldata name) public payable {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the BLOCC name service", "image": "data:image/svg+xml;base64,',
    ...
    _setTokenURI(newRecordId, "ipfs://QmSm3VJKCxyPvevg6371WEK21LpLTTrgvPbS9FDZrsLRWr");
...
function price(string calldata name) public pure returns(uint) {
  ...
  require(len > 0);
  if (len == 3) {
    return 2 * 10**17; // 2 MATIC = 2 000 000 000 000 000 000 (18 decimals). We're going with 0.2 Matic cause the faucets don't give a lot
  } else if (len == 4) {
